...
def recognize_words(line_indicator,word_array,n_lines):
    # Print kata kata yang ditemukan dalam bentuk array
    rec=[]
    for listidx in range(n_lines):
        rec.append([])


    pred=act_model.predict(word_array)
    ...
    for wordidxs in out:
        ...
        # word=''.join(word)
        rec[line_indicator[lw_idx]].append(word)
        lw_idx+=1
    ...
    for listidx in range(n_lines):
        ...
        line = rec[listidx]
        line = ['i' if w is None else w for w in line]
        line=' '.join(line)
        print(line)
